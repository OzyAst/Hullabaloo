            ...
            if (this.hullabaloos !== null) {
                ...
                if (idx !== -1) {
                    ...
                    if (this.hullabaloos.length > 1) {
                        // Граница закрытого алерта
                        top = parseInt(this.hullabaloos[idx].elem.css("top"));
                        next = idx + 1;
                        // находим границу следующего алерта и расчитываем на сколько его подвинуть
                        move = parseInt(this.hullabaloos[next].elem.css("top")) - top;

                        // двигаем все алерты, которые идут за закрытым
                        for (i = idx; i < this.hullabaloos.length; i++) {
                            this.animate(self.hullabaloos[i], parseInt(self.hullabaloos[i].elem.css("top")) - move);
                        }
                    }
